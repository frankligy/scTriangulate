...
def draw_enrich_plots(key):
    # for example, key=gs
    m = pd.read_csv('./marker_{}.txt'.format(key),sep='\t',index_col=0)
    for cluster in m.index:
        ...
        plt.savefig('./diagnose/{0}_{1}_enrichment.png'.format(key,cluster),bbox_inches='tight')
        plt.close()
...
def draw_umap(adata,key):
    ...
    for cluster in m.index:
        ...
        plt.savefig('./diagnose/{0}_{1}_marker_umap.png'.format(key,cluster),bbox_inches='tight')
        plt.close()

    e = pd.read_csv('./exclusive_gene_{}.txt'.format(key),sep='\t',index_col=0)
    for cluster in e.index:
        a = literal_eval(e.loc[cluster,:]['genes'])
        top = a[:10]
        sc.pl.umap(adata,color=top,ncols=5)
        plt.savefig('./diagnose/{0}_{1}_exclusive_umap.png'.format(key,cluster),bbox_inches='tight')
        plt.close()
